...
class OperatorEventController
{

    private const BASE_URL = "views/operator/event";
    private const BASE_MODEL = "EVENT";
    private const ROLES = ["operator", "user"];
    ...
    public static function renderScreen(Request $req, Response $res)
    {
        try {
            ...
            $filtered_event = array_filter($events, function($event) use ($operator_credentials) {
    ...
    public static function renderCreatePage(Request $req, Response $res)
    {
        ...
        // Fetch organization and operator credentials
        $organization = $organizationModel->find();
        $operator_credentials = (new Model("OPERATOR"))->findOne(["USER_ID" => $UID]);
    
        // Filter organizations based on operator credentials
        $filtered_organization = array_filter($organization, function($org) use ($operator_credentials) {
            return $org["ID"] === $operator_credentials["ORGANIZATION_ID"];
        });
    
        // Convert filtered_organization to an array to access it correctly
        $filtered_organization = array_values($filtered_organization);
    
        // Check if the filtered_organization array is not empty
        if (empty($filtered_organization)) {
            // Handle case where no organization is found
            return $res->status(400)->redirect("/event/create", ["error" => "Event already exists"]);
        }
    
        $filtered_organization = $filtered_organization[0];
    
        // Fetch department list and filter based on organization
        $department_list = $departmentModel->find();
        $departmentCredentials = array_filter($department_list, function($department) use ($filtered_organization) {
            return $department["ID"] === $filtered_organization["DEPARTMENT_ID"];
        });
    
        $departmentCredentials = array_values($departmentCredentials);
    
        // Fetch course and organization lists
        $courseCredentials = $courseModel->find([]);
        $organization_list = $organizationModel->find([]);
    
        // Debugging line (can be removed in production)
        // dd($filtered_organization);
    
        // Render the page with the filtered data
        $res->status(200)->render(self::BASE_URL . "/pages/create.page.php", [
            "departmentList" => $departmentCredentials,
            "courseList" => $courseCredentials,
    ...
    public static function createEvent(Request $req, Response $res)
    {
        ...
        $createdEvent = $eventModel->createOne([
            "ID" => $UID,
            ...$credentials,
            "ORGANIZATION_ID" => $operator_credentials["ORGANIZATION_ID"],
            "STATUS" => "ACTIVE"
        ]);
